import numpy as np

from .models import GameMatchResult, TablePoints, TotalClassification, Player, Game, Competition


class PopulateController:

    def populate_matches(self):
        pass

    # def populate_players(self):
    #    player = Player()
    #    player.player_nickname = 'Eversong'
    #    player.player_first_name = 'Daniel'
    #    player.player_last_name = 'Bicho'
    #    player.save()

    def populate_games(self):
        game = Game()
        game.game_complexity = 1.17
        game.game_name = 'Sushi Go'
        game.game_time = 15
        game.game_url = 'https://boardgamegeek.com/boardgame/133473/sushi-go'
        game.save()

        game = Game()
        game.game_complexity = 2.12
        game.game_name = 'Pandemic: Reign of Cthulhu'
        game.game_time = 40
        game.game_url = 'https://www.boardgamegeek.com/boardgame/192153/pandemic-reign-cthulhu'
        game.save()

        game = Game()
        game.game_complexity = 1.03
        game.game_name = 'Dobble'
        game.game_time = 5
        game.game_url = 'https://boardgamegeek.com/boardgame/125048/dobble-free-demo-version'
        game.save()

        game = Game()
        game.game_complexity = 1.67
        game.game_name = 'Dragons & Chickens'
        game.game_time = 20
        game.game_url = 'https://boardgamegeek.com/boardgame/205080/dragons-chickens'
        game.save()

        game = Game()
        game.game_complexity = 1.67
        game.game_name = 'Dragons & Chickens'
        game.game_time = 20
        game.game_url = 'https://boardgamegeek.com/boardgame/205080/dragons-chickens'
        game.save()

        game = Game()
        game.game_complexity = 1.33
        game.game_name = 'Codenames'
        game.game_time = 15
        game.game_url = 'https://boardgamegeek.com/boardgame/178900/codenames'
        game.save()

        game = Game()
        game.game_complexity = 2.35
        game.game_name = 'Catan'
        game.game_time = 120
        game.game_url = 'https://boardgamegeek.com/boardgame/13/catan'
        game.save()

        game = Game()
        game.game_complexity = 3.36
        game.game_name = 'Scythe'
        game.game_time = 115
        game.game_url = 'https://boardgamegeek.com/boardgame/169786/scythe'
        game.save()


class ScoresController:

    def get_player_performance_by_game(self, player, game_name, competition_id=1):
        possible_points = 0
        scored_points = 0
        game_match_results = GameMatchResult.objects.filter(result_player=player,
                                                            result_match_id__match_game__game_name=game_name,
                                                            result_match_id__competition_id=competition_id)

        for match_result in game_match_results:
            table_points = TablePoints.objects.filter(table_standing="1").first()
            game = match_result.result_match_id.match_game
            possible_points += table_points.table_score * game.game_complexity
            # it is assuming that the result_points were generated by generate_score_points
            scored_points += match_result.result_points

        return scored_points / possible_points if scored_points != 0 else 0

    def get_player_performance(self, player, competition_id=1):
        possible_points = 0
        scored_points = 0
        game_match_results = GameMatchResult.objects.filter(result_player=player,
                                                            result_match_id__competition_id=competition_id)
        for match_result in game_match_results:
            table_points = TablePoints.objects.filter(table_standing="1").first()
            game = match_result.result_match_id.match_game
            possible_points += table_points.table_score * game.game_complexity
            # it is assuming that the result_points were generated by generate_score_points
            scored_points += match_result.result_points
        return round(scored_points / possible_points, 2) if scored_points != 0 else 0

    def generate_score_points(self):
        game_match_results = GameMatchResult.objects.all()

        for result in game_match_results:
            match = result.result_match_id
            game = match.match_game
            table_points = TablePoints.objects.filter(table_standing=result.result_standing).first()
            if game and table_points:
                result.result_points = round(table_points.table_score * game.game_complexity, 2)
                result.save()

    def generate_total_classification(self, competition_id=1):
        classifications = []
        players_performance = []
        # for player, generate score
        players = Player.objects.all()
        players_score = []
        for player in players:
            score = 0
            game_match_results = GameMatchResult.objects.filter(result_player=player,
                                                                result_match_id__competition_id=competition_id)
            for match in game_match_results:
                score += match.result_points

            players_score.append(score)
            players_performance.append(self.get_player_performance(player, competition_id))

        sorted_indexes = np.argsort(players_score)[::-1]

        for i in range(len(sorted_indexes)):
            classification = TotalClassification()
            classification.player = players[int(sorted_indexes[i])]
            classification.standing_position = i + 1
            classification.score_points = round(players_score[int(sorted_indexes[i])], 2)
            classification.performance_points = round(players_performance[int(sorted_indexes[i])], 2)
            competition = Competition.objects.filter(competition_id=competition_id).first()
            classification.competition_id = competition
            classification.save()
            classifications.append(classification)

        return classifications

    def generate_game_standings(self, game_name, competition_id):
        standings = []
        players_performance = []
        players = Player.objects.all()
        players_score = []

        for player in players:
            score = 0
            game_match_results = GameMatchResult.objects.filter(result_match_id__match_game__game_name=game_name,
                                                                result_match_id__competition_id=competition_id,
                                                                result_player=player)
            for match in game_match_results:
                score += match.result_points

            players_score.append(score)
            players_performance.append(self.get_player_performance_by_game(player, game_name, competition_id))

        sorted_indexes = np.argsort(players_score)[::-1]

        for i in range(len(sorted_indexes)):
            classification = TotalClassification()
            classification.player = players[int(sorted_indexes[i])]
            classification.standing_position = i + 1
            classification.score_points = round(players_score[int(sorted_indexes[i])], 2)
            classification.performance_points = round(players_performance[int(sorted_indexes[i])], 2)
            standings.append(classification)

        return standings
